...
def age_testing(input_image,output_file):

    model = load_cnn()

    category = {0: 'Adult', 1: 'Unknown', 2: 'fetus', 3: 'Child', 4: 'Not Applicable'}
    ...
    if ("age_predict" in keys):
        data1["age_predict"] = str(category[label])

    if ("age_predict_perc" in keys):
        data1["age_predict_perc"] = str(prob) + '%'
