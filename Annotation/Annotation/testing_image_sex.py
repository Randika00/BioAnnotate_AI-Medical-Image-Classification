...
def load_cnn():
    ...
    model.load_weights('image_sex.h5')

    return model

def image_sex_testing(input_image,output_file):

    model = load_cnn()

    category = {0: 'Female', 1: 'Male', 2: 'Unknown', 3: 'Not applicable'}
    ...
    if ("image_sex_predict" in keys):
        data1["image_sex_predict"] = str(category[label])

    if ("image_sex_predict_perc" in keys):
        data1["image_sex_predict_perc"] = str(prob) + '%'
